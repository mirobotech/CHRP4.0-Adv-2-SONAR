/*==============================================================================
 ...
 Attaching a SONAR module to CHRP4
 A 4-pin HC-SR04 sonar module can be connected either across the header pins
 near the floor sensors of the CHRP4 circuit board or to any two sets of header
 pins on CHRP4. A set of header pins contains three electrical contacts arranged
 in a commonly used servo configuration: pin 1 (square pad) - an input or output
 signal connected to the microcontroller I/O lines, pin 2 (round pad) - the +5V
 supply (or Vcc) to power an external device, and pin 3 (round pad) - electrical
 ground (or GND). The four HC-SR04 module connections are often labelled as Vcc,
 TRIG, ECHO, and GND.
 
 This example program is configured to work with a SONAR module connected to a
 header soldered across the header pins, using the H2 signal pin for the TRIG
 output, and the H3 signal pin as the ECHO input. Vcc and GND are supplied by
 the Vcc and GND connections on H1 and H4, respectively. The program can be
 configured to work using other header pins -- consult the schematic to check
 which header connections are shared by other components of your CHRP4 build.
==============================================================================*/
...
int main(void)
{
    ...
    // Enable output on TRIG pin (match TRISB settings to pin definitions)
    TRISBbits.TRISB5 = 0;       // Set H2 (TRIG) as output pin (H3 remains input)
 ...
 * 
 * 1.   The TRISB register controls the PORTB data tristate buffers. What
 *      state must a TRIS bit be in to allow its port pin to be an output?
 * 
 * 2.   This program uses a TRISBbits operator to modify the H2 TRIS bit, but
 *      a logical operation such as the following could have been used instead:

    TRISB = TRISB & 0b11011111; // Set H2 (TRIG) as output, leave H3 as input

 *      Describe an advantage of using a logical operation to selectively clear
 *      TRIS bits over using individual TRISBbits operations.
 * 
